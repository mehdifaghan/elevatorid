...
export default function Transfers() {
  ...
  const fetchTransfers = async () => {
    try {
      ...
      // Fetch transfers
      const transfersResponse = await realApiRequest.get('/user/transfers');
      const transfers = transfersResponse.data.transfers || [];
      setTransfers(transfers);
      setStats(calculateStats(transfers));

    } catch (error: any) {
      console.error('Error fetching transfers:', error);
      toast.error('خطا در بارگذاری اطلاعات انتقالات');
  ...
  const calculateStats = (transfersList: Transfer[]): TransferStats => {
    return {
      ...
      incoming: transfersList.filter(t => t.direction === 'incoming').length,
      outgoing: transfersList.filter(t => t.direction === 'outgoing').length
    };
  };
  ...
  const handleCreateTransfer = async (transferData: any) => {
    try {
      const response = await realApiRequest.post('/user/transfers', transferData);
      if (response.data) {
        await fetchTransfers(); // Refresh the list
        toast.success('انتقال جدید با موفقیت ثبت شد');
      }
    } catch (error) {
      console.error('Error creating transfer:', error);
      toast.error('خطا در ثبت انتقال جدید');
    }
  };
    ...
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">در حال بارگذاری انتقالات...</p>
        </div>
      </div>
    );
  }
      ...
      {showSellModal && selectedTransferForSale && (
        <SellModal
          ...
          preSelectedPart={{
            ...
            code: selectedTransferForSale.partSerial,
            serial: selectedTransferForSale.partSerial,
            category: 'قطعات الکتریکی'
          }}
          title="فروش قطعه"
          description="انجام فرآیند فروش قطعه از طریق انتقال"
        />
      )}
    </div>
  );
